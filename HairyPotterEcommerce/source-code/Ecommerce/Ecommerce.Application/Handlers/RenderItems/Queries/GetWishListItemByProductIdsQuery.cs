...
public class GetWishListItemByProductIdsQuery : IRequest<IEnumerable<WishListDto>>
{
    public long[] ProductIds { get; set; }
}
public class GetWishListItemByProductIdsQueryHandler : IRequestHandler<GetWishListItemByProductIdsQuery, IEnumerable<WishListDto>>
{
    private readonly IDataContext _db;
    private readonly IMapper _mapper;
    private readonly IKeyAccessor _keyAccessor;
    private readonly ICookieService _cookie;
    public GetWishListItemByProductIdsQueryHandler(IDataContext db, IMapper mapper, IKeyAccessor keyAccessor, ICookieService cookie)
    {
        _db = db;
        _mapper = mapper;
    ...
    public async Task<IEnumerable<WishListDto>> Handle(GetWishListItemByProductIdsQuery request, CancellationToken cancellationToken)
    {
        ...
        var getShop = await GetShopList(outOfStockThreshold, request.ProductIds);
        var newProduct = getShop.GroupBy(w => w.Id, (key, value) => new WishListDto
        {
            ...
            CategorySlug = value.First().CategorySlug,
