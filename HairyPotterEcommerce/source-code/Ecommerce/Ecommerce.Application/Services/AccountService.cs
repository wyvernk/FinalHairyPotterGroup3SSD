...
public class AccountService : IAccountService
{
    ...
    /*
    public async Task<Response<UserDto1>> SignInAsync(LoginUserDto loginUserDto)
    {
        ...
        if (user == null)
        {
            return Response<UserDto1>.Fail("Username does not exist");
        }

        // Verify password using the stored salt
        var passwordHasher = new PasswordHasher();
        if (!passwordHasher.VerifyPassword(loginUserDto.Password, user.PasswordHash, user.SecurityStamp))
        {
            return Response<UserDto1>.Fail("Invalid login attempt");
        }

        // Check if email is confirmed
        if (!user.EmailConfirmed)
        {
            return Response<UserDto1>.Fail("Email is not confirmed");
        }
        ...
        var claims = new List<Claim>
    {
        ...
        return Response<UserDto1>.Success(new UserDto1 { Id = user.Id }, "Login successful");
    }
